...
export default {
  ...
  session: {
    ...
    warningAfterInactiveMinutes: Number(get('SESSION_WARNING_AFTER_INACTIVE_MINUTES', 50)),
  ...
  apis: {
    ...
    aapApi: {
      url: get('AAP_API_URL', 'http://localhost:8080', requiredInProduction),
      healthPath: '/health/ping',
      timeout: {
        response: Number(get('AAP_API_TIMEOUT_RESPONSE', 5000)),
        deadline: Number(get('AAP_API_TIMEOUT_DEADLINE', 5000)),
      },
    ...
    arnsApi: {
      url: get('ARNS_API_URL', 'http://localhost:9091', requiredInProduction),
      healthPath: '/health/ping',
      timeout: {
    ...
    riskActuarialApi: {
      url: get('RISK_ACTUARIAL_API_URL', 'http://host.docker.internal:9092', requiredInProduction),
      healthPath: '/health/ping',
      timeout: {
