...
const strengthsYesDetails = GovUKCharacterCount({
  ...
  label: 'Give details',
  maxLength: 1425,
  dependentWhen: Answer('accommodation_practitioner_analysis_strengths_or_protective_factors').match(
...
const riskOfSeriousHarmYesDetails = GovUKCharacterCount({
  ...
  label: 'Give details',
  maxLength: 1425,
  dependentWhen: Answer('accommodation_practitioner_analysis_risk_of_serious_harm').match(Condition.Equals('YES')),
  validWhen: [
    validation({
      condition: not(Self().not.match(Condition.IsRequired())),
      message: 'Enter details',
...
const riskOfSeriousHarmNoDetails = GovUKCharacterCount({
  ...
  label: 'Give details (optional)',
  maxLength: 1425,
  dependentWhen: Answer('accommodation_practitioner_analysis_risk_of_serious_harm').match(Condition.Equals('NO')),
})
...
const riskOfReoffendingYesDetails = GovUKCharacterCount({
  ...
  label: 'Give details',
  maxLength: 1000,
  dependentWhen: Answer('accommodation_practitioner_analysis_risk_of_reoffending').match(Condition.Equals('YES')),
  validWhen: [
    validation({
      condition: not(Self().not.match(Condition.IsRequired())),
      message: 'Enter details',
...
const riskOfReoffendingNoDetails = GovUKCharacterCount({
  ...
  label: 'Give details (optional)',
  maxLength: 1000,
  dependentWhen: Answer('accommodation_practitioner_analysis_risk_of_reoffending').match(Condition.Equals('NO')),
})
