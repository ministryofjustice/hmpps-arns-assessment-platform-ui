...
const optionalFutureDateValidations = () => {
  return [
    validation({
      when: and(hasAnyDatePart(), Self().not.match(Condition.Object.PropertyHasValue('day'))),
      message: 'Date must include a day',
      details: { field: 'day' },
    }),
    validation({
      when: and(hasAnyDatePart(), Self().not.match(Condition.Object.PropertyHasValue('month'))),
      message: 'Date must include a month',
      details: { field: 'month' },
    }),
    validation({
      when: and(hasAnyDatePart(), Self().not.match(Condition.Object.PropertyHasValue('year'))),
      message: 'Date must include a year',
      details: { field: 'year' },
    }),
    validation({
      when: hasAllDateParts(),
...
const shortTermEndDate = GovUKDateInputFull({
  ...
  validate: optionalFutureDateValidations(),
})
