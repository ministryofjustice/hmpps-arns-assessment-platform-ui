...
export const victimCards = CollectionBlock({
  collection: Data('victims').each(
    Iterator.Map(
      HtmlBlock({
        content: [
          Modal({id: Loop.Index0(), title: contentFor('are_you_sure_you_want_to_delete'), buttonText: commonContentFor('delete')}),
          GovUKSummaryList({
            card: {
              title: { text: contentFor('victim_card_title', commonContentFor(Format('ordinals.%1', Loop.Index()) as any)).pipe(Transformer.String.Capitalize()) },
              actions: {
                items: [
                  {
