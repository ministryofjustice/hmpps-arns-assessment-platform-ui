import { validation, Self, Format, Answer, or, and, not, when } from '@form-engine/form/builders'
...
export const drugsReasonsForUse = GovUKCheckboxInput({
  ...
  validate: [
    validation({
      when: and(anyDrugUsedInLastSixMonths, Self().not.match(Condition.IsRequired())),
      message: 'Select why they use drugs',
    }),
    validation({
      when: and(not(anyDrugUsedInLastSixMonths), Self().not.match(Condition.IsRequired())),
