...
export const drugsSummaryCards = (drugValue: ChainableExpr<PipelineExpr>) => GovUKSummaryList({
  card: {
    title: {
      ...
      value: {
        blocks: [
          ...
          GovUKBody({
            ...
            visibleWhen:
              and(
                Answer(Format(Question.drug_last_used_value, SANGenerators.getDrugValueLower(drugValue))).match(Condition.Equals(Option.more_than_six)),
                Answer(Question.drugs_injected).match(Condition.Array.Contains(drugValue)),
              )
          }),
        ]
    ...
    content: [
      CollectionBlock({
        collection: Data('drugsUsedInLastSix').each(
          Iterator.Map(
            drugsSummaryCards(Item().path('value')),
          ),
        ),
      })
    ]
...
export const notUsedInLastSixMonthsSection = TemplateWrapper({
  ...
  slots: {
    ...
    content: [
      CollectionBlock({
        collection: Data('drugsUsedMoreThanSix').each(
          Iterator.Map(
            drugsSummaryCards(Item().path('value')),
          ),
        ),
  ...
  rows: [
    {
      ...
      value: {
        blocks: [
          GovUKBody({
            text: SANGenerators.getTextFromListDefinition(receivingTreatmentField.items, Answer(Question.receiving_treatment)),
          }),
          GovUKBody({
      ...
      value: {
        blocks: [
          ...
          GovUKBody({
            ...
            visibleWhen: and(
              Answer(Question.drugs_reasons_for_use).match(Condition.IsRequired()),
              Answer(Question.drugs_reasons_for_use).match(Condition.Array.Contains(Option.curiosity_or_experimentation)),
            )
          }),
          GovUKBody({
      ...
      value: {
        blocks: [
          ...
          GovUKBody({
            ...
            size: "s",
            visibleWhen: Answer(Question.drug_use_changes).match(Condition.Equals(CommonOption.wants_to_make_changes_knows_how_to)),
          }),
          GovUKBody({
            text: Answer(Question.wants_to_make_changes_needs_help_drugs_details),
            size: "s",
            visibleWhen: Answer(Question.drug_use_changes).match(Condition.Equals(CommonOption.wants_to_make_changes_needs_help)),
          }),
          GovUKBody({
            text: Answer(Question.thinking_about_making_changes_drugs_details),
            size: "s",
            visibleWhen: Answer(Question.drug_use_changes).match(Condition.Equals(CommonOption.thinking_about_making_changes)),
          }),
          GovUKBody({
            text: Answer(Question.does_not_want_to_make_changes_drugs_details),
            size: "s",
            visibleWhen: Answer(Question.drug_use_changes).match(Condition.Equals(CommonOption.does_not_want_to_make_changes)),
          }),
          GovUKBody({
            text: Answer(Question.does_not_want_to_answer_drugs_details),
            size: "s",
            visibleWhen: Answer(Question.drug_use_changes).match(Condition.Equals(CommonOption.does_not_want_to_answer)),
          }),
        ],
      },
      actions: {
...
export const drugsPractitionerAnalysisMotivatedToStop = GovUKRadioInput({
  ...
  items: [
    { value: Option.no_motivation, text: contentFor('question.drugs_practitioner_analysis_motivated_to_stop.option.NO_MOTIVATION') },
    { value: Option.partial_motivation, text: contentFor('question.drugs_practitioner_analysis_motivated_to_stop.option.PARTIAL_MOTIVATION') },
    { value: Option.full_motivation, text: contentFor('question.drugs_practitioner_analysis_motivated_to_stop.option.FULL_MOTIVATION') },
...
const riskOfSeriousHarmYesDetails = GovUKCharacterCount({
  ...
  label: commonContentFor('required_details'),
  maxLength: 1425,
  dependentWhen: Answer(Question.drug_use_practitioner_analysis_risk_of_serious_harm).match(Condition.Equals(CommonOption.yes)),
  validWhen: [
    validation({
      condition: not(Self().not.match(Condition.IsRequired())),
      message: commonContentFor('validation.enter_details'),
...
const riskOfSeriousHarmNoDetails = GovUKCharacterCount({
  ...
  label: commonContentFor('optional_details'),
  maxLength: 1425,
  dependentWhen: Answer(Question.drug_use_practitioner_analysis_risk_of_serious_harm).match(Condition.Equals(CommonOption.no)),
})
