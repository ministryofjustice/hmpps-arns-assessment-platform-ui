 ...
 */
const logTierOutcome = (logger: SentencePlanEffectsDeps['logger'], crn: string, httpStatus: number) => {
  if (httpStatus === 200) {
    return
  }
...
export const loadSupervisionPackage = (deps: SentencePlanEffectsDeps) => async (context: SentencePlanContext) => {
  ...
  if (!crn) {
    deps.logger.error('Cannot load supervision package: missing CRN in session')
  ...
  if (tierResult.status === 'fulfilled') {
    logTierOutcome(deps.logger, crn, tierResult.value.httpStatus)
    context.setData('tierCalculation', tierResult.value.calculation)
  } else {
    deps.logger.error({ err: tierResult.reason, crn }, 'Failed to fetch tier details from MPoP Components API')
  }
  ...
  if (packageResult.status === 'rejected') {
    ...
    deps.logger.error({ err: packageResult.reason, crn }, 'Failed to load supervision package')
  } else if (packageResult.value) {
    context.setData('supervisionPackageDetails', packageResult.value)
  } else {
    deps.logger.info({ crn }, 'No supervision package for this person yet')
  }
}
