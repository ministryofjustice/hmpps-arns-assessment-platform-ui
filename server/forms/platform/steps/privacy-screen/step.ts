...
import { PlatformEffects } from '../../effects'
...
export const privacyScreenStep = step({
  ...
  view: {
    template: 'platform/views/platform-form-step',
    locals: {
  ...
  onAccess: [
    accessTransition({
      effects: [PlatformEffects.loadPrivacyScreenSessionData()],
      next: [
        redirect({
  ...
  onSubmission: [
    submitTransition({
      ...
      onValid: {
        effects: [
          PlatformEffects.loadPrivacyScreenSessionData(),
          PlatformEffects.setPrivacyAccepted(),
          PlatformEffects.sendPrivacyScreenAuditEvent(),
        ],
