...
function getOrderedKeys(left: Record<string, unknown>, right: Record<string, unknown>): string[] {
  ...
  if (isObject(left) && isObject(right)) {
    ...
    for (const key of keys) {
      ...
      if (!hasLeft) {
        ...
        children[key] = diffJson(left[key], right[key])
      }
    }
...
function renderDiff(node: DiffNode, indent = 0, key?: string): string {
  const pad = '&nbsp;'.repeat(indent * 2)

  switch (node.type) {
    case 'unchanged':
      return `<div class="line unchanged">${pad}${key ? `"${key}": ` : ''}${renderValue(node.value)}</div>`

    case 'added':
      return `<div class="line added">${pad}+ ${key ? `"${key}": ` : ''}${renderValue(node.value)}</div>`

    case 'removed':
      return `<div class="line removed">${pad}- ${key ? `"${key}": ` : ''}${renderValue(node.value)}</div>`

    case 'changed':
      return `
<div class="line removed">${pad}- ${key ? `"${key}": ` : ''}${renderValue(node.before)}</div>
<div class="line added">${pad}+ ${key ? `"${key}": ` : ''}${renderValue(node.after)}</div>`

    case 'object': {
      const children = Object.entries(node.children)
        .map(([childKey, childNode]) => renderDiff(childNode, indent + 1, childKey))
        .join('')

      return `
<div class="line unchanged">${pad}${key ? `"${key}": ` : ''}{</div>
${children}
<div class="line unchanged">${pad}}</div>`
    ...
    case 'array': {
      const children = node.children
        .map(child => renderDiff(child, indent + 1))
        .join('')
