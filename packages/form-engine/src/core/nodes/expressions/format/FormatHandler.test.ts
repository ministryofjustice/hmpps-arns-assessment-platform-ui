...
describe('FormatHandler', () => {
  ...
  describe('evaluate', () => {
    ...
    describe('HTML escaping', () => {
      it('should escape ampersand in Reference arguments', async () => {
        // Arrange - Reference nodes contain user data that needs escaping
        const refNode = ASTTestFactory.reference(['data', 'goalTitle'])
        const formatNode = createFormatNode('<h1>%1</h1>', [refNode])
        const mockContext = createMockContext()
        const invoker = createMockInvoker({
          returnValueMap: new Map([[refNode.id, 'Drugs & alcohol support']]),
        })
        const handler = new FormatHandler(formatNode.id, formatNode)
      ...
      it('should escape angle brackets in Reference arguments', async () => {
        // Arrange - XSS attempt via Reference (user input)
        const refNode = ASTTestFactory.reference(['answers', 'userInput'])
        const formatNode = createFormatNode('<p>%1</p>', [refNode])
        const mockContext = createMockContext()
        const invoker = createMockInvoker({
          returnValueMap: new Map([[refNode.id, '<script>alert("xss")</script>']]),
        })
        const handler = new FormatHandler(formatNode.id, formatNode)
      ...
      it('should escape quotes in Reference arguments', async () => {
        // Arrange - quotes in user input from Reference
        const refNode = ASTTestFactory.reference(['data', 'stepDescription'])
        const formatNode = createFormatNode('<span>%1</span>', [refNode])
        const mockContext = createMockContext()
        const invoker = createMockInvoker({
          returnValueMap: new Map([[refNode.id, 'Complete "thinking skills" programme']]),
        })
        const handler = new FormatHandler(formatNode.id, formatNode)
      ...
      it('should preserve user-typed HTML entities by double-escaping', async () => {
        // Arrange - user intentionally typed "&amp;" in a Reference field
        const refNode = ASTTestFactory.reference(['data', 'goalTitle'])
        const formatNode = createFormatNode('<h2>Goal: %1</h2>', [refNode])
        const mockContext = createMockContext()
        const invoker = createMockInvoker({
          returnValueMap: new Map([[refNode.id, 'Learn about R&amp;D processes']]),
        })
        const handler = new FormatHandler(formatNode.id, formatNode)
      ...
      it('should handle complex user input with special characters', async () => {
        // Arrange - realistic user input with multiple special characters from Reference
        const refNode = ASTTestFactory.reference(['data', 'goalTitle'])
        const formatNode = createFormatNode('<h2>%1</h2>', [refNode])
        const mockContext = createMockContext()
        const invoker = createMockInvoker({
          returnValueMap: new Map([
            [
      ...
      it('should not escape the template, only Reference arguments', async () => {
        // Arrange - template has HTML tags, Reference arguments have user content
        const refNode1 = ASTTestFactory.reference(['data', 'goalUuid'])
        const refNode2 = ASTTestFactory.reference(['data', 'goalTitle'])
        const formatNode = createFormatNode('<a href="/goal/%1">View %2</a>', [refNode1, refNode2])
        const mockContext = createMockContext()
        const invoker = createMockInvoker({
          returnValueMap: new Map([
            [refNode1.id, 'abc-123'],
            [refNode2.id, 'Health & wellbeing goal'],
          ]),
        })
        const handler = new FormatHandler(formatNode.id, formatNode)
        ...
        // Assert - template HTML preserved, only Reference values escaped
        expect(result.value).toBe('<a href="/goal/abc-123">View Health &amp; wellbeing goal</a>')
      })

      it('should NOT escape primitive string arguments (trusted source code)', async () => {
        // Arrange - primitive strings come from source code, not user input
        const formatNode = createFormatNode('<div class="%1">%2</div>', [
          'govuk-error-message',
        ...
        // Assert - primitive strings are NOT escaped (they're trusted source code)
        expect(result.value).toBe('<div class="govuk-error-message"><span>Error text</span></div>')
      })

      it('should escape Reference but not primitive in mixed arguments', async () => {
        // Arrange - mix of trusted source code and user data
        const refNode = ASTTestFactory.reference(['data', 'userName'])
        const formatNode = createFormatNode('<li class="%1">Welcome, %2</li>', ['govuk-list__item', refNode])
        const mockContext = createMockContext()
        const invoker = createMockInvoker({
          returnValueMap: new Map([[refNode.id, 'Tom & Jerry']]),
        })
        const handler = new FormatHandler(formatNode.id, formatNode)
        ...
        // Assert - primitive class name NOT escaped, Reference user name IS escaped
        expect(result.value).toBe('<li class="govuk-list__item">Welcome, Tom &amp; Jerry</li>')
      })
    })
  })
})
