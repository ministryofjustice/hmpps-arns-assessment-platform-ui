...
describe('NotWiring', () => {
  ...
  describe('wire()', () => {
    it('should wire operand to NOT node', () => {
      // Arrange
      const operandNode = ASTTestFactory.predicate(LogicType.TEST)

      const notNode = ASTTestFactory.predicate(LogicType.NOT, {
        operand: operandNode,
      } as any)
    ...
    it('should wire multiple NOT nodes', () => {
      ...
      const notNode1 = ASTTestFactory.predicate(LogicType.NOT, {
        operand: operand1,
      } as any)

      const notNode2 = ASTTestFactory.predicate(LogicType.NOT, {
        operand: operand2,
      } as any)
