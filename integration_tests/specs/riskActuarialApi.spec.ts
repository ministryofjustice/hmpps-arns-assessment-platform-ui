...
test.describe('Risk Actuarial API', () => {
  test('can get scores via the Risk Actuarial API', async ({ riskActuarialApiBuilder }) => {
    ...
    const builder = riskActuarialApiBuilder
      ...
      .withMostRecentOffenceDate('1994-01-01')
      ...
      .withIsCurrentOffenceAgainstVictimStranger(false)
    ...
    // Act
    const riskScores = await builder.save()

    // Assert
    expect(riskScores).toEqual(expectedResponse)
  })
})
